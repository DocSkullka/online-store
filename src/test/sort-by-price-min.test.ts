...
import { sortByPriceMin } from "../filter/compose/sort-by-price-min";
...
const res = [items[2], items[1], items[0], items[4], items[3]];
describe("sort 5 items", () => {
  filterStore.sort = "pricemin";
  test("sort from max price to min price", () => {
    expect(sortByPriceMin(items)).toEqual(res);
  });
});
